...
class CommentController extends Controller
{
    ...
    public function create(InsertCommentRequest $request): JsonResponse
    {
        ...
        if (Auth::user() && !boolval(Auth::user()->can_reply) && $valid->get('id') !== null && !boolval(Auth::user()->is_admin)) {
            return $this->json->errorBadRequest(['permission is not allowed']);
        }
        ...
        if (Auth::user() && !empty(Auth::user()->is_filter)) {
            $valid->comment = Aman::factory()->masking($valid->comment, ' * ');
        }

        $user_id = Auth::id();
        if(!$user_id) {
            $user = User::find($valid->get('name'), 'name');

            if (!$user->exist()) {
                $user = User::create([
                    'name' => $valid->get('name'),
                    'email' => $valid->get('name').'@gmail.com',
